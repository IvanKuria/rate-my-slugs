...
async function processInstructor(row, instructorName, department = null) {
  ...
  try {
    ...
    const fullClassName = courseDept && courseCode ? `${courseDept} ${courseCode}` : null;
    
    console.log(`📤 Fetching data for: ${instructorName} (Department: ${department}, Class: ${fullClassName})`);
    ...
    let gradeData = null;
    if (rmpData && rmpData.status === 'success' && rmpData.matchedName && fullClassName) {
...
async function fetchGradeData(matchedName, className) {
  if (!className || !matchedName) {
    console.log('📊 No class name or matched name available for grade data lookup');
    return null;
  }
